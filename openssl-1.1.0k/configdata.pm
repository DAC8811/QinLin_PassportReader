...
our %config = (
  ...
  builddir => ".",
  cc => "gcc",
  cflags => " -fPIC -ffast-math -ftree-vectorize -fdata-sections -ffunction-sections",
  ...
  openssldir => "",
  options => "--prefix=/home/gwi/桌面/passport/QiLin_ARM_aarch64_IR/install/ enable-shared -fPIC -ffast-math -ftree-vectorize -fdata-sections -ffunction-sections -Wl,--gc-sections no-asan no-crypto-mdebug no-crypto-mdebug-backtrace no-ec_nistp_64_gcc_128 no-egd no-fuzz-afl no-fuzz-libfuzzer no-heartbeats no-md2 no-msan no-rc5 no-sctp no-ssl-trace no-ssl3 no-ssl3-method no-ubsan no-unit-test no-weak-ssl-ciphers no-zlib no-zlib-dynamic",
  perl => "/usr/bin/perl",
  perlargv => [ "--prefix=/home/gwi/桌面/passport/QiLin_ARM_aarch64_IR/install/", "linux-aarch64", "shared", "-fPIC", "-ffast-math", "-ftree-vectorize", "-fdata-sections", "-ffunction-sections", "-Wl,--gc-sections" ],
  prefix => "/home/gwi/桌面/passport/QiLin_ARM_aarch64_IR/install/",
  processor => "",
