...
//		imgCopy = imgCopy(Rect(0, imgCopy.rows * 3 / 4, imgCopy.cols, imgCopy.rows - imgCopy.rows * 3 / 4));
	...
	if (type == 0 || type == -1) {
		...
		resize(smallImg, smallImg, Size(imgCopy.cols / 2, imgCopy.rows / 2));//smallImg为imgCopy大小的1/2
		smallImg = smallImg(Rect(smallImg.cols / 6, smallImg.rows * 4 / 5, smallImg.cols * 4 / 6, smallImg.rows - smallImg.rows * 4 / 5));
		...
		imgCopy = imgCopy(Rect(0, imgCopy.rows * 3 / 4 - blackHeight, imgCopy.cols, imgCopy.rows - imgCopy.rows * 3 / 4));//上移blackHeight
