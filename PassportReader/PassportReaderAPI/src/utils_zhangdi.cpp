...
std::string processMRZ(std::string mrzCode, DOC_TYPE docType, std::string checkResult, std::string checkDetails,Json::Value& root,std::map<std::string, std::string>& mapinfor){
    if(mrzCode.empty()||mrzCode.length() == 0)
        return "";
    std::string jsonStr;
    switch(docType) {
        case PASSPORT: {
            jsonStr = processPassportJson(mrzCode,"",checkResult,checkDetails,docType,0,root);
            break;
        }
        case HK_MACAO_PASSPORT: {
            jsonStr = processHKTWJson(mrzCode,"",checkResult,checkDetails,docType,0,root);
            break;
        }
        case HK_MACAO_VISIT_MAINLAND: {
            jsonStr = processHXZJson(mrzCode,"",checkResult,checkDetails,docType,0,mapinfor);
            break;
        }
        case TAIWAN_VISIT_MAINLAND:{
            jsonStr = processTWMJson(mrzCode,"",checkResult,checkDetails,docType,0,root);
            break;
        }
...
std::string processDG(std::string DG1, std::string DG11, DOC_TYPE docType, std::string checkResult, std::string checkDetails,Json::Value& root,std::map<std::string, std::string>& mapinfor){
    if(DG1.empty()||DG1.length()==0)
        return "";
    std::string jsonStr;
    switch (docType){
        case PASSPORT: {
            jsonStr = processPassportJson(DG1,DG11,checkResult,checkDetails,docType,1,root);
            break;
        }
        case HK_MACAO_PASSPORT: {
            jsonStr = processHKTWJson(DG1,DG11,checkResult,checkDetails,docType,1,root);
            break;
        }
        case HK_MACAO_VISIT_MAINLAND: {
            jsonStr = processHXZJson(DG1,DG11,checkResult,checkDetails,docType,1,mapinfor);
            break;
        }
        case TAIWAN_VISIT_MAINLAND: {
            jsonStr = processTWMJson(DG1,DG11,checkResult,checkDetails,docType,1,root);
            break;
        }
        case TAIWAN_PASSPORT:{
            jsonStr = processHKTWJson(DG1,DG11,checkResult,checkDetails,docType,1,root);
...
std::string processHXZJson(std::string mrzCode, std::string DG11,std::string checkResult, std::string checkDetails,DOC_TYPE docType, int chipStatus,std::map<std::string, std::string>& mapinfor){
    std::stringstream json;
    std::string jsonStr;
    std::string info = delN(mrzCode);//把OCR字符串中间的换行符号去除
    std::string firstLine = info.substr(0,30);
    ...
    if(mrzCode.substr(0,2).compare("C<")==0){
        ...
        json << "{"
             ...
             << "\"FullName\":\"" << DG11 << "\","
             << "\"RenewalTimes\":\"" << "" << "\","
             << "\"hxcIDHKMNu\":\"" << mapinfor["hxcIDHKMNu"] << "\","
             //<< "\"AuthDetails\":\"" << checkDetails <<"\""
             << "\"MRZ1\":\"" << firstLine << "\","
             << "\"MRZ2\":\"" << secondLine << "\","
             << "\"MRZ3\":\"" << nameLine << "\","
             << "\"RfIDMRZ\":\"" << firstLine + secondLine + nameLine << "\","
             << "\"hxcSignDate\":\"" << mapinfor["hxcSignDate"] << "\""
             << "}"
             << "}" << std::ends;
        jsonStr = json.str();
        return jsonStr;
    ...
    json << "{"
         ...
         << "\"Issuing\":\"" << mapinfor["hxcSignDepart"]  << "\","
         << "\"LastName\":\"" << lastname << "\","
         << "\"MiddleName\":\"" << "" << "\","
         << "\"Nationality\":\"" << "CHN" << "\","
         << "\"Option1\":\"" << hkid << "\","
         ...
         << "\"hxcIDHKMNu\":\"" << secondLine.substr(19, 7) << "(" << secondLine.substr(26, 1) << ")" << "\","
         //<< "\"AuthDetails\":\"" << checkDetails <<"\""
         << "\"MRZ1\":\"" << firstLine << "\","
         << "\"MRZ2\":\"" << secondLine << "\","
         << "\"MRZ3\":\"" << nameLine << "\","
...
std::string processTWMJson(std::string mrzCode, std::string DG11, std::string checkResult, std::string checkDetails,DOC_TYPE docType, int chipStatus,Json::Value& PassportInfo){
    std::string info = delN(mrzCode);//把OCR字符串中间的换行符号去除
    std::string firstLine = info.substr(0,30);
    std::string secondLine = info.substr(30,30);
    std::string thirdLine = info.substr(60,30);
...
//         << "}" << std::ends;
    ...
    root["ChipReadResult"] = std::to_string(chipStatus);
    root["DocumentType"] = std::to_string(docType);
    root["IDcardInfo"] = "";
    PassportInfo["BirthDate"] = birthPre + firstLine.substr(23, 6);
    PassportInfo["DocType"] = "C";
    if(!firstname.empty())
        PassportInfo["FirstName"] = firstname;
    PassportInfo["Gender"] = GENDER[firstLine.substr(22, 1)];
    if(PassportInfo["Issuing"] == "")
        PassportInfo["Issuing"] = thirdLine.substr(26,4);
    if(!lastname.empty())
        PassportInfo["LastName"] = lastname;
    PassportInfo["MiddleName"] = "";
    PassportInfo["Nationality"] = "CHN";
    PassportInfo["Option1"] = twid;
    PassportInfo["SerialNum"] = firstLine.substr(2, 8);
    PassportInfo["Validity"] = validPre + firstLine.substr(15, 6);
    //PassportInfo["Authenticy"] = checkResult;
    if(!DG11.empty())
        PassportInfo["FullName"] = DG11;
    PassportInfo["RenewalTimes"] = firstLine.substr(12,2);
    //PassportInfo["AuthDetails"] = checkDetails;
...
std::string processPassportJson(std::string mrzCode, std::string DG11, std::string checkResult, std::string checkDetails,DOC_TYPE docType, int chipStatus,Json::Value& PassportInfo){
...
//         << "}" << std::ends;
    ...
    root["DocumentType"] = std::to_string(docType);
    root["IDcardInfo"] = "";
    PassportInfo["BirthDate"] = birthPre + secondLine.substr(13, 6);
    PassportInfo["DocType"] = "P";
    if(!firstname.empty())
        PassportInfo["FirstName"] = firstname;
    PassportInfo["Gender"] = GENDER[secondLine.substr(20, 1)];
    if(PassportInfo["Issuing"] == "")
        PassportInfo["Issuing"] = firstLine.substr(2, 3);
    if(!lastname.empty())
        PassportInfo["LastName"] = lastname;
    PassportInfo["MiddleName"] = "";
    PassportInfo["Nationality"] = secondLine.substr(10, 3);
    if(PassportInfo["Option1"].isNull())
        PassportInfo["Option1"] = "";
    PassportInfo["SerialNum"] = secondLine.substr(0, 9);
    PassportInfo["Validity"] = validPre + secondLine.substr(21, 6);
    //PassportInfo["Authenticy"] = checkResult;
    if(!DG11.empty())
        PassportInfo["FullName"] = DG11;
    if(PassportInfo["RenewalTimes"].isNull())
...
std::string processHKTWJson(std::string mrzCode, std::string DG11, std::string checkResult, std::string checkDetails,DOC_TYPE docType, int chipStatus,Json::Value& PassportInfo){
//    std::stringstream json;
    std::string jsonStr;
    ...
    if(chipStatus==1){
        ...
        PassportInfo["MRZ2"] = secondLine;
        PassportInfo["MRZ3"] = thirdLine;
        PassportInfo["RfIDMRZ"] = mrzCode;
...
//         << "}" << std::ends;
    ...
    root["DocumentType"] = std::to_string(docType);
    root["IDcardInfo"] = "";
    PassportInfo["BirthDate"] = birthPre + firstLine.substr(21, 6);
    PassportInfo["DocType"] = "C";
    if(!firstname.empty())
        PassportInfo["FirstName"] = firstname;
    if(!gender.empty())
        PassportInfo["Gender"] = gender;
//    if(PassportInfo["Issuing"] == "")
//        PassportInfo["Issuing"] = "";
    if(!lastname.empty())
        PassportInfo["LastName"] = lastname;
    PassportInfo["MiddleName"] = "";
    if(PassportInfo["Nationality"].isNull())
        PassportInfo["Nationality"] = "";
    if(PassportInfo["Option1"].isNull())
        PassportInfo["Option1"] = "";
    PassportInfo["SerialNum"] = firstLine.substr(2, 9);
    PassportInfo["Validity"] = validPre + firstLine.substr(13, 6);
    //PassportInfo["Authenticy"] = checkResult;
    if(!DG11.empty())
        PassportInfo["FullName"] = DG11;
    if(PassportInfo["RenewalTimes"].isNull())
