...
//    char GetResult(EF_NAME efName,unsigned char *retData,unsigned int *retLen);
    ...
    char AuthKeyFunc(unsigned char* authKeys);
    //------------------------------------------------------------------------------
    ...
    char ReadHardwareVersion(std::string &version);
...
private:
    ...
    char ReadEchipInfo(std::string &codetonfc, RF_SPEED s);

    char SetPassportComeIn(RF_SPEED s);
