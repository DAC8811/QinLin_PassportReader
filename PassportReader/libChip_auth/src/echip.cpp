...
char CChipReader::SetPassportComeIn(RF_SPEED s) {
    ...
    for (int i = 0; i < 5; i++) {
        ret = echipReader.RF_14443_Reset(0, cpInfo, &cpLen, s);
...
char CChipReader::ReadEchipInfo(std::string &codetonfc, RF_SPEED s) {
    std::string Kenc;
    std::string Kmac;
    std::string RND_ICC;
    std::string RND_IFD;
...
char CChipReader::AuthKeyFunc(unsigned char* authKeys){
    ...
    ret = Crypto_3DES_ECB(CRYPTO_ENCRYPT,authKeys, RND_NUM.size(), rnd);
    if(ret != 0){
        LOGE("设备认证：随机数加密失败！");
...
char CChipReader::AuthFunc(std::string path){
    unsigned char encrypt_keys[16]={0};
    unsigned char decrypt_keys[16]={0};
    const char* keystring = "this is a key string";
    DES_cblock keys;
    DES_key_schedule key_schedule;
    DES_string_to_key(keystring, &keys);
    if(DES_set_key_checked(&keys, &key_schedule) != 0){
        LOGE("密钥校验失败");
        return false;
    }
    DES_cblock ivec;
    ...
    if(Input){
        Input.read((char*)encrypt_keys, 16);
        Input.close();
    } else{
        LOGE("密钥文件打开失败");
        return false;
    ...
    DES_cbc_encrypt(encrypt_keys, decrypt_keys, 16, &key_schedule, &ivec, DES_DECRYPT);
    printf("\n");
    for(int i=0;i<16;i++)
          printf("%d:%x ",i,decrypt_keys[i]);
    printf("\n");
    return AuthKeyFunc(decrypt_keys);
}
...
char CChipReader::ReadChipUID(unsigned char* chipUID){
    ...
    LOGI("读取ChipUID编号成功!");
    LOG(INFO)<<"读取ChipUID编号成功!"<<endl;
    return true;
}

char CChipReader::ReadHardwareVersion(std::string &version){
    std::string request("\x00",1);//读取设备自定义编号的命令
    std::string response;
    char ret = this->PostPassportCommand(RF_CMD_READ_CHIP_UID, request, version);
    if(!ret){
        LOGE("读取固件版本号失败！");
        LOG(ERROR)<<"读取固件版本号失败！"<<endl;
        return false;
    }
    //memcpy(chipUID, response.data(),response.size());

    LOGI("读取C固件版本号成功!");
    LOG(INFO)<<"读取固件版本号成功!"<<endl;
    return true;
}
...
char CChipReader::ReadInfo(std::string &code) {
    ...
    try {
        ret = this->ReadEchipInfo(codetonfc, s);
        if(ret != true){
            //降速再读一次
            s = RF_SPEED_212K;
            ret = this->ReadEchipInfo(codetonfc, s);
...
int  CChipReader::PassportCardTest(void){
	...
	memset(cpInfo, 0, sizeof(cpInfo));
	int retInt = -1;
    retInt = echipReader.RF_14443_Reset(0xFF, cpInfo, &cpLen, RF_SPEED_848K); //软复位
