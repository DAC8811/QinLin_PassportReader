...
unsigned int
RFReader::RF_14443_Reset(unsigned char cPara, unsigned char *cpInfo, unsigned char *cpLen, RF_SPEED s) {
    int ret = 0;
...
RFReader::RF_IDCard_ReadFinger(unsigned char *sInfo, unsigned int * sLength, unsigned char *cpPhoto, unsigned int *ipLength, unsigned char *cpFinger, unsigned int *ipFLength){
    ...
    tempBuf = (unsigned char *) malloc(1024 * 4);
    ret = m_usbHid.controlReadData(tempBuf, 4096);
    if (ret < 0) {
        LOGE("Error:usb can't read!");
        free(tempBuf);
        return RF_ERR_FAILURE;
    }
    unsigned char t[4096] = {0};
    memcpy(t, tempBuf, ret);
    ...
    if (recvLen > 0) {
        ...
        //256 + 1024 = 1280
        //256 + 1024 + 1024 = 2304
        if(recvLen < 1600){//1600是我自己定义的，介于有指纹2314和无指纹之间1290的一个值
            //没有指纹
            memcpy(sInfo, (void *) (tempBuf + RF_RECV_IDCARD_INFO_POS1), RF_RECV_IDCARD_INFO_LEN);
            *sLength = RF_RECV_IDCARD_INFO_LEN;
            *ipLength = MIN(recvLen - RF_RECV_IDCARD_INFO_LEN - 1, 1024);
            //std::cout << "ipLength == " << *ipLength << std::endl;
            memcpy(cpPhoto, (void *) (tempBuf + RF_RECV_IDCARD_PIC_POS1), *ipLength);
        }else{
            //有指纹
            memcpy(sInfo, (void *) (tempBuf + RF_RECV_IDCARD_INFO_POS1), RF_RECV_IDCARD_INFO_LEN);
            *sLength = RF_RECV_IDCARD_INFO_LEN;
            *ipLength = RF_RECV_IDCARD_PHOTO_LEN;
            memcpy(cpPhoto, (void *) (tempBuf + RF_RECV_IDCARD_PIC_POS1), *ipLength);
            *ipFLength = MIN(recvLen - RF_RECV_IDCARD_INFO_LEN - RF_RECV_IDCARD_PHOTO_LEN - 1, 1024);
            memcpy(cpFinger, (void *) (tempBuf + RF_RECV_IDCARD_FINGER_POS1), *ipFLength);
        }
